...
const Button = ({ children, variant = "primary", ...props }:any) => {
    ...
    return (
        <button
            {...props}
            className={`${variants[variant]} cursor-pointer rounded-md border px-2 md:px-4 py-0.5 md:py-1.5  text-xs/6 md:text-sm/6 font-medium transition-colors ${props.className}`}
        >
            {children}
        </button>
    )
