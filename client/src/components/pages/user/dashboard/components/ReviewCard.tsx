  ...
  return (
    ...
    >
      <div className="flex gap-4">
        <Image
          src={`/uploads/categoryIcons/${response?.feedbackForm?.category?.iconUrl}.png`}
          width={200}
          height={200}
          alt={""}
        ...
        {response.details?.pricipalName ? (
          <div className="flex flex-col leading-snug gap-0">
            <div className="text-lg font-semibold">
              Principal {response.details.pricipalName}
            </div>
            <div className="text-base font-normal">
              {response.details.schoolName} {response.details.schoolCountry}
            </div>
          </div>
        ) : response.details?.directorName ? (
          <div className="flex flex-col">
            <div className="text-lg font-semibold">
              Pricipal {response.details.directorName}
            </div>
            <div className="text-lg font-semibold">
      ...
      <a
        className="text-base text-muted-foreground font-normal flex gap-2 items-center mt-auto"
    ...
    <div className="flex gap-1 items-center justify-start">
      {Array.from({ length: 10 }).map((_, i) => (
        <AppleIcon
          key={v4()}
          size={16}
          fill={i + 1 < rating ? "red" : "gray"}
          stroke={i + 1 < rating ? "red" : "gray"}
        />
      ))}
