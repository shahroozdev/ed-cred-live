...
const PackageCard = ({plan}:{plan:Record<string, any>}) => {
  ...
  return (
    <div className={`p-6 rounded-4xl flex flex-col shadow-md bg-white text-black ${plan.primary ? "md:scale-115" : ""}`}>
            ...
            <button className="w-full mt-auto rounded-2xl bg-green-600 text-white py-3 hover:shadow-md hover:opacity-90 cursor-pointer" onClick={()=>router.push(`/purchase/${plan.title}`)}>Buy Now</button>
        </div>
  )
}
